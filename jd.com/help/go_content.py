...
def main():	
	...
	for index,k in enumerate(content_url_list):		
		...
		try:
			...
			title = str(s.find('title').get_text()).split('-')[0].decode('utf-8')
			content = str(s.find('div',attrs={"class":"wanecon"}))
			#匹配所有字符\s匹配任意的空白符（包括空格，制表符(Tab)，换行符，中文全角空格）\S则是任意不是空白符的字符
			content = re.sub(r'<div align=[\s\S]*','',content).decode('utf-8')
			...
			#content_txt = str(s.find('div',attrs={"class":"contxt"}).get_text()).decode('utf-8')[0:250]#取前250个字符
			'''
			s2 = BeautifulSoup(content,"xml")
			content_txt = s2.get_text()[0:250]#取前250个字符
			
			#替换掉引号和空格、换行等，英文引号会影响描述标签，空格、换行等会影响json里的答案字段
			content_txt = re.sub(r'[\s\'\"]*?','',content_txt)

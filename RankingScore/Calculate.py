...
with conn:
	...
	print '写入完毕'	
	#然后用shell对域名文件domain.txt进行排序计数去重...
subprocess.call(["cat domain.txt|sort|uniq -c|sort -r|head -100 > sortedTop100Domain.txt"],shell='true')	
	#上一个命令不行的话用下面这个...
	#os.system('cat result.txt|sort|uniq -c|sort -r > result')	

#下面代码是加权算排名,域名在每个关键词的前十排名得分为权数值

domain_list = []
for line in open('sortedTop100Domain.txt'):
	line = line.strip()
	domain = line.split()[-1]
